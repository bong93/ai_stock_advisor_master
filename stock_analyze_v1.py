...
def analyze_with_llama3(ticker_name, custom_event): # 👈 news_text 매개변수 삭제
    try:
        ...
        [⚠️ 엄격한 분석 규칙 ⚠️]
        1. 무리한 논리 금지: 해당 이슈가 '{ticker_name}'의 핵심 사업모델(매출/이익)과 직접적이고 강한 연관성이 없다면, 억지로 연결 고리를 지어내지 마십시오.
        2. 거시적 영향 우선: 특정 산업의 이슈라도, 그것이 인플레이션 완화 등 글로벌 거시 경제 전반에 긍정적이라면 기술주/대형주에는 호재로 평가하십시오.
        3. 연관성이 극히 낮거나 판단할 수 없다면 SCORE를 0으로 주고, 그 이유를 설명하십시오.
