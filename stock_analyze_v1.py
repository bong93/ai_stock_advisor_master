...
def analyze_with_llama3(ticker_name, custom_event): # 👈 news_text 매개변수 삭제
    try:
        ...
        입력된 [이슈]가 한국 증시의 '{ticker_name}' 종목 단기 주가(1~2주)에 미칠 영향을 분석하세요.

        [분석 규칙]
        1. '{ticker_name}'의 핵심 주력 사업(매출원)을 파악하고, 이슈가 이에 직접적인 타격을 주는지 분석하십시오.
        2. 연관성이 극도로 낮거나 억지로 논리를 짜맞춰야 하는 경우(예: IT기업과 농산물 가격 등), 무조건 0점을 부여하십시오.
        3. 어떠한 부연 설명이나 인사말도 하지 말고, 오직 아래의 [출력 포맷]에 맞춰 정확히 2줄로만 대답하십시오.
        4. 거시 경제 고려: 특정 산업 이슈라도 금리 인하, 인플레이션 완화 등 시장 전체의 투자 심리를 개선한다면 대형주에 긍정적 점수를 줄 수 있습니다.
        5. 직접 연관이 없더라도, 인플레이션 하락/금리 인하 등 거시 경제(매크로) 전반의 투자 심리를 개선하는 글로벌 호재라면 대형주에 긍정적 점수(+2 ~ +5)를 부여하십시오.

        [예시 1 - 직접적 연관성이 있는 경우]
        이슈: 글로벌 빅테크 데이터센터 설비투자 40% 축소
        종목: 삼성전자
        SCORE: -8
        REASON: 데이터센터 투자 축소는 삼성전자의 핵심 매출원인 서버용 메모리 반도체(HBM, DRAM)의 직접적인 수요 감소와 가격 하락을 유발하므로 큰 악재입니다.

        [예시 2 - 연관성이 없는 노이즈인 경우]
        이슈: 브라질 가뭄으로 인한 글로벌 옥수수 가격 사상 최고치 폭등 및 식량 위기
        종목: 삼성전자
        SCORE: 0
        REASON: 농산물 가격 폭등은 삼성전자의 주력 사업인 반도체, 스마트폰, 가전의 수요 및 원가 구조와 직접적인 연관성이 없어 펀더멘털에 미치는 영향이 없습니다.

        [실제 분석]
        이슈: {custom_event if custom_event else "특이사항 없음"}
        종목: {ticker_name}
        
        [출력 포맷] (반드시 아래 형식으로만 답변할 것)
        SCORE: 
        REASON: 
        """
        ...
        reason = reason_match.group(1).strip() if reason_match else "분석 결과를 파싱할 수 없습니다."
