...
def analyze_with_llama3(ticker_name, custom_event): # 👈 news_text 매개변수 삭제
    try:
        ...
        [분석 규칙]
        1. '{ticker_name}'의 핵심 주력 사업을 파악하고, 이슈가 이에 직접 타격을 주는지 분석하십시오.
        2. 연관성이 없거나 억지로 엮어야 하는 산업 무관 노이즈라면 0점을 주십시오.
        3. 직접 연관이 없더라도, 인플레이션 하락/금리 인하 등 거시 경제(매크로) 전반의 투자 심리를 개선하는 글로벌 호재라면 대형주에 긍정적 점수(+2 ~ +5)를 부여하십시오.
        4. 오직 아래의 [출력 포맷]에 맞춰 정확히 2줄로만 대답하십시오.
        ...
        이슈: 브라질 가뭄으로 인한 글로벌 옥수수 가격 폭등
        종목: 삼성전자
        SCORE: 0
        REASON: 농산물 가격 폭등은 삼성전자의 주력 사업인 반도체, 스마트폰의 수요 및 원가 구조와 전혀 연관성이 없어 펀더멘털에 미치는 영향이 없습니다.

        [예시 3 - 거시 경제(매크로) 간접 호재인 경우]
        이슈: 미국-이란 전쟁 종식으로 인한 국제 유가 급락
        종목: 삼성전자
        SCORE: +4
        REASON: 유가 하락은 직접적인 원가 영향은 적으나, 글로벌 인플레이션 우려를 완화시켜 기술주에 우호적인 금리 환경을 조성하고 IT 전방 수요 회복 심리를 자극하므로 긍정적입니다.
