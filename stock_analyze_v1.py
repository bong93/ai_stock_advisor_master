...
def analyze_with_llama3(ticker_name, custom_event): # 👈 news_text 매개변수 삭제
    try:
        ...
        [⚠️ 퀀트 분석 사고 프로세스 (반드시 이 순서대로 생각할 것) ⚠️]
        1. 기업 분석: 먼저 '{ticker_name}' 기업의 핵심 사업 모델과 주력 매출원(예: 반도체, 스마트폰, 자동차, 금융 등)이 무엇인지 속으로 식별하십시오.
        2. 역학 관계 추론: [글로벌 매크로 이슈]가 앞서 식별한 '{ticker_name}'의 주력 매출원의 수요, 공급, 원가 등에 직접적인 타격을 주거나 이익을 주는지 논리적으로 연결해 보십시오. (예: 데이터센터 투자는 메모리 반도체 수요와 직결됨)
        3. 노이즈 필터링: 만약 1, 2번을 거쳤음에도 사업 모델과 전혀 무관한 이슈라면(예: IT기업과 농산물 가격) 억지로 엮지 말고 반드시 0점을 부여하십시오.
